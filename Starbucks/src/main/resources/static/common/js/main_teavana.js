$(document).ready(function() {
    ...
    if (myWin > 959) {
        ...
        autAni.append(TweenMax.to($('.main-visual_slogan'), .5, {
            css: {
                opacity: 1
            }
        }));
        ...
        for (var i = 1; i <= setCnt; i++) {
            ...
            autAni.append(TweenMax.to($('.set_' + tempIdx), .5, {
                css: {
                    opacity: 1
                }
            }));
        }
        autAni.append(TweenMax.to($('.btn_slogan'), .5, {
            css: {
                opacity: 1
            }
        }));
    ...
    if (myWin > 641) {
        ...
        controller.addTween(10, TweenMax.to($('.bean_img_box'), .3, {
            opacity: 0,
            ease: Quad.easeOut
        }), 1, 0);
        controller.addTween(10, TweenMax.from($('.bean_img_box'), 2.0, {
            left: '-100%',
            opacity: 0,
            ease: Quad.easeOut
        }), 1, 0);
        controller.addTween(10, TweenMax.to($('.bean_txt_box'), .3, {
            opacity: 0,
            ease: Quad.easeOut
        }), 1, 0);
        controller.addTween(10, TweenMax.from($('.bean_txt_box'), 2.5, {
            right: '-100%',
            opacity: 0,
            ease: Quad.easeOut
        }), 1, 0);
